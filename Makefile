...
clean-mockgen: ; rm */*_mock.go
clean-data: ; rm -rf ./.scootdata/*
clean-go: ; go clean ./...
default: ; go build $$(go list ./... | grep -v /vendor/)
dependencies:
...
generate: ; go generate $$(go list ./... | grep -v /vendor/)
format: ; go fmt $$(go list ./... | grep -v /vendor/)
vet: ; go vet $$(go list ./... | grep -v /vendor/)
coverage: ; sh testCoverage.sh $(TRAVIS_FILTER)
# Runs only unit tests and property tests
test-unit-property: ; go test -race -timeout 120s -tags=property_test $$(go list ./... | grep -v /vendor/ | grep -v /cmd/) $(TRAVIS_FILTER)
# Runs only unit tests
# Only invoked manually so we don't need to modify output.
test-unit: ; go test -race -timeout 120s $$(go list ./... | grep -v /vendor/ | grep -v /cmd/)
...
# Only invoked manually so we don't need to modify output.
test-integration: ; go test -race -timeout 120s -tags="integration property_test" $$(go list ./... | grep -v /vendor/ | grep -v /cmd/)
swarmtest:
...
thrift-worker: ; cd workerapi && thrift --gen go:package_prefix=github.com/scootdev/scoot/workerapi/gen-go/,package=worker,thrift_import=github.com/apache/thrift/lib/go/thrift worker.thrift && cd ..
# Create generated code in github.com/scootdev/scoot/sched/gen-go/... from job_def.thrift
thrift-sched: ; cd sched && thrift --gen go:package_prefix=github.com/scootdev/scoot/sched/gen-go/,package=schedthrift,thrift_import=github.com/apache/thrift/lib/go/thrift job_def.thrift && cd ..
# Create generated code in github.com/scootdev/scoot/scootapi/gen-go/... from scoot.thrift
thrift-scoot: ; cd scootapi && thrift --gen go:package_prefix=github.com/scootdev/scoot/scootapi/gen-go/,package=scoot,thrift_import=github.com/apache/thrift/lib/go/thrift scoot.thrift && cd ..
